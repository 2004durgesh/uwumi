...
const EpisodeList = ({ mediaType, provider, id,swipeable=false }: { mediaType: string; provider: string; id: string,swipeable?:boolean }) => {
  const swipeRef = useRef<SwipeableMethods>(null);
  const router = useRouter();
  const theme = useTheme();
  const currentPlayingEpisode = useCurrentPlayingEpisode((state) => state.currentPlayingEpisode);
  console.log('currentPlayingEpisode', currentPlayingEpisode);
  ...
  const renderPressableItem = ({ item }: { item: any }) => {
    ...
    >
      <YStack gap={'$4'} padding={2} borderWidth={1} borderColor={currentPlayingEpisode===item.id?'$color4':'$background'} backgroundColor='$background'>
        <XStack gap={'$4'}>
          ...
          <YStack padding={2} flex={1}>
            ...
            <XStack justifyContent="space-between" alignItems="center">
              <Text fontSize="$2.5">{currentPlayingEpisode===item?.id?"Now Playing":null}</Text>
